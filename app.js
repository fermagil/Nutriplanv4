...
import { app, db, auth, provider } from './js/firebase-config-module.js';
import { getFirestore, collection, addDoc, getDocs, query, where, orderBy, limit, doc, getDoc, setDoc, deleteDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/11.7.3/firebase-firestore.js";
...
console.log('Firebase initialized from centralized config');
...
// Export Firebase instances for use in other modules (re-export from config)
...
// ============================================
// FICHA MÉDICA DEPORTIVA - Firestore Functions
// ============================================

/**
 * Helper: Get client name from their most recent "toma"
 * @param {string} clienteId 
 * @returns {Promise<string|null>}
 */
async function getClientNameFromTomas(clienteId) {
    try {
        const tomasRef = collection(db, `clientes/${clienteId}/tomas`);
        const tomasQuery = query(tomasRef, orderBy('fecha', 'desc'), limit(1));
        const tomasSnap = await getDocs(tomasQuery);

        if (!tomasSnap.empty) {
            const latestToma = tomasSnap.docs[0].data();
            return latestToma.nombre || null;
        }
        return null;
    } catch (error) {
...
/**
 * Fetch medical record for a specific client
 * @param {string} clienteId - Firestore document ID of the client
 * @returns {Promise<Object|null>} Medical record data or null
 */
export async function fetchFichaMedica(clienteId) {
    if (!clienteId) {
    ...
    try {
        // Path: clientes/{clienteId}/fichaMedica/latest
        const fichaMedicaRef = doc(db, `clientes/${clienteId}/fichaMedica/latest`);
        const fichaMedicaSnap = await getDoc(fichaMedicaRef);

        if (fichaMedicaSnap.exists()) {
            const data = fichaMedicaSnap.data();
            console.log('✅ Ficha Médica found:', data);
            return {
                id: clienteId,
                ...data
            };
        } else {
            console.log('ℹ️ No Ficha Médica exists for this client yet');
            // Return structure with client ID and name from tomas
            const clientName = await getClientNameFromTomas(clienteId);
            return {
                id: clienteId,
                nombreCliente: clientName || '',
                // All other fields will be undefined/empty
            };
        }
    } catch (error) {
        console.error('❌ Error fetching Ficha Médica:', error);
        return null;
    }
}

/**
 * Save/Update medical record for a client
 * @param {string} clienteId - Client ID
 * @param {Object} fichaMedicaData - Complete medical record data
 * @returns {Promise<boolean>} Success status
 */
export async function saveFichaMedica(clienteId, fichaMedicaData) {
    if (!clienteId) {
    ...
    try {
        ...
        // Save to Firestore
        await setDoc(fichaMedicaRef, dataToSave, { merge: true });

        console.log('✅ Ficha Médica saved successfully');
        return true;
    } catch (error) {
        console.error('❌ Error saving Ficha Médica:', error);
        throw error;
    }
}

/**
 * Delete medical record for a client
 * @param {string} clienteId 
 * @returns {Promise<boolean>}
 */
export async function deleteFichaMedica(clienteId) {
    if (!clienteId) {
        throw new Error('No client ID provided');
    }
    ...
    try {
        const fichaMedicaRef = doc(db, `clientes/${clienteId}/fichaMedica/latest`);
        await deleteDoc(fichaMedicaRef);
        console.log('✅ Ficha Médica deleted successfully');
        return true;
    } catch (error) {
        console.error('❌ Error deleting Ficha Médica:', error);
        throw error;
    }
}

// Make functions globally accessible for other scripts
window.fetchFichaMedica = fetchFichaMedica;
window.saveFichaMedica = saveFichaMedica;
window.deleteFichaMedica = deleteFichaMedica;
